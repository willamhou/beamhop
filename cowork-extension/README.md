...
This MCPB exposes captures already stored by Beamhop to Claude Desktop and desktop-backed Cowork sessions.

## Features

- Fetch a Capture by its `cap_…` id.
- Fetch the latest Capture when no id is supplied.
...
Then double-click `cowork-extension/dist/beamhop.mcpb`, review its permissions, and install it. Keep Beamhop's default database at `~/Library/Application Support/beamhop/inbox.sqlite`.
